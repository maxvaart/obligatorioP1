...
function ocultarTodo() {
  ...
  document.querySelector("#visualizadorEntregasProfe").style.display = "none";
...
function muestraEntregasTotalProfe() {
  ...
  document.querySelector("#visualizadorEntregasProfe").style.display = "block";
...
function muestraIniProfe() {
  ...
  document.querySelector("#inicio").innerHTML = generarEjercicios(ejerciciosFiltrados);
...
function muestraPerfil() {
  ...
  if (tipoUsuario == "profesor") {
    ...
    document.querySelector("#profePerfil").innerHTML = `${usuarioLogueado.profesor.nombre} ${usuarioLogueado.profesor.apellido}`;
    ...
    datosHTML += `
      ...
      <table style="width:100%">
        ...
        <tbody class="unaEntrega">
          <tr>
            <td><span class="nombreAlumnoEntrega">${entrega.alumnoAutor.nombre} ${entrega.alumnoAutor.apellido}</span></td>
...
function visualizarEjerciciosFiltrados() {
  ...
  if(arrayEjFiltrados.length == 0) {
    ...
    for(ejercicio of arrayEjFiltrados){
      htmlMuestra += `<h3>${ejercicio.nombre}</h3><hr>`;
      let arrayAux = [];
      arrayAux.push(ejercicio);
      htmlMuestra += generarEjercicios(arrayAux);
      htmlMuestra += `<br><br>`;
      htmlMuestra += generarEntregas(ejercicio.entregas);
      htmlMuestra += `<br><br>`;
    }
  }
  document.querySelector("#entregasProfe").innerHTML = htmlMuestra;
}